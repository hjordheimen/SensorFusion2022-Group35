...
class ModelIMU:
    ...
    def predict_nom(self,
        ...
        """
        a, w            = x_est_nom.ori.as_rotmat() @ (z_corr.acc) + self.g, z_corr.avel
        ...
        kappa           = dt*w
        delta_rot       = RotationQuaterion.from_avec(kappa)  
        ori_pred        = x_est_nom.ori.multiply(delta_rot) 

        acc_bias_pred   = x_est_nom.accm_bias - self.accm_bias_p*np.eye(3) @ x_est_nom.accm_bias*dt 
        gyro_bias_pred  = x_est_nom.gyro_bias - self.gyro_bias_p*np.eye(3) @ x_est_nom.gyro_bias*dt 
    ...
    def get_error_G_c(self,
        ...
        Args:
            x_est_nom: previous nominal state
        Returns:
            G_c (ndarray[15, 15]): G in (10.68) --> Should be 15 x 12.
        """
        G_c                 = np.zeros((15, 12))
        Rq                  = x_est_nom.ori.as_rotmat()
    ...
    def get_discrete_error_diff(self,
        ...
        A_d             = VanLoanMatrix[15:30, 15:30].T 
        GQGT_d          = VanLoanMatrix[0:15, 15:30]  
    ...
    def predict_err(self,
        ...
        Q               = Ad @ GQGTd
        P_pred          = Ad @ x_est_prev_err.cov @ Ad.T + Q 
